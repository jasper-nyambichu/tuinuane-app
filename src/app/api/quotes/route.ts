...
export async function POST(req: NextRequest) {
  ...
  // ✅ FIX 1: Use adminClient (service role) to bypass RLS on public inserts
  const { data, error } = await adminClient
    .from('quotes')
    .insert([{
      name: body.name,
  ...
  if (error) {
    console.error('[quotes/POST] DB error:', error.message)
    return NextResponse.json({ error: error.message }, { status: 500 })
  }

  // ✅ FIX 2: Wrap emails in try/catch so a Resend failure doesn't kill the response
  try {
    ...
    console.error('[quotes/POST] Email error:', emailError)
  }
